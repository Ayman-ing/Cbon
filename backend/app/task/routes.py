...
def get_all_tasks_from_db(db: Session, skip: int = 0, limit: int = 100, status_filter: Optional[str] = None, project_id: Optional[UUID] = None) -> List[Task]:
...
def get_all_tasks(
    ...
    """
    tasks = get_all_tasks_from_db(db, skip=skip, limit=limit, status_filter=status_filter, project_id=project_id)
...
def delete_task(
    ...
    if soft:
        ...
        deleted = delete_task_by_id(db, task_id)
        if not deleted:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Task with id {task_id} not found"
